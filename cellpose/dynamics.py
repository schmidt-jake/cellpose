...
@torch.jit.script
def get_masks(p: torch.Tensor, iscell=None, rpad=20):
    ...
    if iscell is not None:
        if dims == 3:
            inds = torch.meshgrid(
                torch.arange(shape0[0]),
                torch.arange(shape0[1]),
                torch.arange(shape0[2]),
                indexing="ij",
            )
        elif dims == 2:
            inds = torch.meshgrid(
    ...
    h, _ = torch.histogram(torch.stack(pflows), bins=edges)
    hmax = h.copy()
    for i in range(dims):
...
def compute_masks(
    ...
    if (
        ...
        dynamics_logger.info("No cell pixels found.")
        shape = resize if resize is not None else cellprob.shape
        mask = torch.zeros(shape, dtype=torch.int16)
        p = torch.zeros((len(shape), *shape), torch.int16)
        return mask, p
